...
const skillCategories = [
  ...
  },
  {
    ...
    icon: Wrench,
    description: 'A sharp toolbelt for modern software delivery — from design to deployment.',
    items: [
      { name: 'Git & GitHub',      level: 'Expert' },
      { name: 'Figma / Adobe XD', level: 'Proficient' },
      { name: 'Postman / Insomnia',level: 'Advanced' },
      { name: 'VS Code / Vim',     level: 'Expert' },
...
export function Skills() {
  return (
    <>
      <section id="skills" className="py-20 lg:py-32 px-6 md:px-12 w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 lg:gap-20 relative overflow-hidden">

        {/* Background Decor — contained inside overflow-hidden */}
        <div className="absolute bottom-0 left-0 w-[50%] h-[50%] bg-accent/5 blur-[80px] rounded-full pointer-events-none -translate-x-1/3 translate-y-1/3" />
        ...
        <div className="w-full lg:w-2/5">
          <div className="lg:sticky lg:top-[30vh] flex flex-col space-y-8 pb-12 lg:pb-0 z-20">
            ...
            <p className="text-text-muted text-lg font-light leading-relaxed max-w-sm">
              I don't just write code. I architect digital ecosystems designed for speed, scale, and
              emotional impact.
            </p>

            <div className="flex gap-4 pt-4">
              <div className="w-12 h-12 neo-convex rounded-full flex items-center justify-center text-accent">
                <Code2 size={20} />
              </div>
              <div className="w-12 h-12 neo-convex rounded-full flex items-center justify-center text-accent">
                <Cpu size={20} />
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 relative z-10">
                {category.items.map((skill) => (
                  ...
                  >
                    <span className="font-sans font-bold text-text text-xs sm:text-sm group-hover/pill:text-accent transition-colors leading-tight">
                      {skill.name}
                    </span>
      ...
      <div className="w-full px-6 md:px-12">
        <div className="max-w-7xl mx-auto h-px bg-gradient-to-r from-transparent via-dark-shadow/20 to-transparent" />
      </div>
    </>
  );
}
